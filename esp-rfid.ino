...
bool activateZummer = false;
bool activeGuardTime = false;
bool inAPMode = false;
int zummerPin = D0;
int zummerTime;
int guardTime;
...
// indicate success and turn off AP
...
void onSTADisconnected(WiFiEventStationModeDisconnected event_info) {
  Serial.println();
  Serial.print(F("[ INFO ] Disconnected from SSID: "));
...
void loop() {
  ...
  if (currentMillis - previousMillis >= zummerTime && activateZummer) {
    activateZummer = false;
    digitalWrite(zummerPin, HIGH);
  }

  if (currentMillis - previousMillis >= guardTime && activeGuardTime) {
    activeGuardTime = false;
  }  
...
void sendTime() {
  ...
  root["lastsync"] = NTP.getLastNTPSync();
  size_t len = root.measureLength();
  AsyncWebSocketMessageBuffer * buffer = ws.makeBuffer(len); //  creates a buffer (len + 1) for you.
  if (buffer) {
    root.printTo((char *)buffer->get(), len + 1);
...
void sendStatus() {
  ...
  root["firmware"] = String(FW_VERSION_STRING);
  root["compile_time"] = String(__DATE__ " " __TIME__);
...
bool loadConfiguration() {
  ...
  guardTime = json["gtime"];
  guardTime += activateTime;
  relayPin = json["rpin"];
  ...
  server.on("/vcard", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    if (request->params() == 1){
      ...
      if (strcmp (p_uid->name().c_str(), "uid") == 0){
        AsyncWebServerResponse *response = request->beginResponse(200, "text/plain", "vcard read success!");
        request->send(response);
        checkUID(p_uid->value(), "vcard");
      }
    }
    else {
      AsyncWebServerResponse *response = request->beginResponse(200, "text/plain", "vcard read error!");
      request->send(response);      
    }
  });  
